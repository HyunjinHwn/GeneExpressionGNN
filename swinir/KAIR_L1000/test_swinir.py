...
from utils import utils_image as uim
...
def main():
    ...
    parser.add_argument('--folder_output', type=str, default='result')
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        uim.imwrite(output, f'{save_dir}/{imgname}.csv')
...
def define_model(args):
    ...
    if args.task == 'classical_sr':
        ...
        param_key_g = 'params'
    elif args.task == 'SwinIR':
        model = net(upscale=args.scale, in_chans=1, img_size=args.training_patch_size, window_size=4,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='pixelshuffle', resi_connection='1conv')
        param_key_g = 'params'
...
def get_image_pair(args, path):
    ...
    # 006 JPEG compression artifact reduction (load gt image and generate lq image on-the-fly)
        ...
        img_gt = uim.imread_uint(path)
        img_lq = uim.imread_uint(f'{args.folder_lq}/{imgname}{imgext}')
