...
IMG_EXTENSIONS = ['.csv', '.CSV'] # Allow .csv or .CSV as valid input; otherwise reject
...
def imssave(imgs, img_path):
    ...
    for i, img in enumerate(imgs):
        ...
        new_path = os.path.join(os.path.dirname(img_path), img_name+str('_{:04d}'.format(i))+'.csv')
        np.savetxt(new_path, np.squeeze(img), delimiter=',')
...
def imsave(img, img_path):
    ...
    np.savetxt(img_path, img, delimiter=',')
...
def tensor2uint(img):
    ...
    # return np.uint8((img*255.0).round())  forced not to return unsigned int, instead return float value directly
    return img
...
# changed default out type to float32
...
def calculate_pcc(img1, img2, border=0):
...
def calculate_scc(img1, img2, border=0):
    x = np.reshape(img1, img1.shape[0]*img1.shape[1])
    y = np.reshape(img2, img2.shape[0]*img2.shape[1])
